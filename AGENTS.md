...
<!-- gitnexus:start -->
# GitNexus — Mandatory Code Intelligence
...
→ process trace when relevant
```

HIGH/CRITICAL impact must be reported before editing. Never rename symbols with blind find/replace; use GitNexus rename preview. Before commit/completion run `gitnexus_detect_changes()` and verify only expected scope changed.
